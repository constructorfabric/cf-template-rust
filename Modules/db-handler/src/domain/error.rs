use {{ crate_name }}_sdk::ProductError;
...
impl From<DomainError> for ProductError {
    fn from(domain_error: DomainError) -> Self {
        match domain_error {
            DomainError::NotFound { id } => ProductError::not_found(id),
            DomainError::Validation { field, message } => {
                ProductError::validation(format!("{field}: {message}"))
            }
            DomainError::Database { .. } | DomainError::InternalError => ProductError::internal(),
        }
    }
}
