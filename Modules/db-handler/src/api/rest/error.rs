...
fn domain_error_to_canonical(e: &DomainError) -> CanonicalError {
    match e {
        DomainError::NotFound { id } => {
            ErrorCode::product_not_found_v1().as_canonical(format!("Product with id {id} was not found"))
        }
        DomainError::Validation { .. } => ErrorCode::product_validation_v1().as_canonical(format!("{e}")),
        DomainError::Database { .. } => {
            tracing::error!(error = ?e, "Database error occurred");
            ErrorCode::product_internal_database_v1().as_canonical("An internal database error occurred")
        }
